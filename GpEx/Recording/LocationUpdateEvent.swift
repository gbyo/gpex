...
/// value.
///
/// An update can carry a location, a set of diagnostics, or both. PhotoTrack never
...
/// Location on", and "can the background activity continue". PhotoTrack observes
...
nonisolated struct SessionDiagnostic: Sendable, Equatable {
    ...
    var serviceSessionRequired: Bool = false
    var authorizationRequestInProgress: Bool = false
    /// Precise Location is off for PhotoTrack. Reported by the service session only.
